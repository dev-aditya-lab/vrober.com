...
export default function CleaningServices() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    api
      .get('/services', { params: { category: 'Cleaning', limit: 10 } })
      .then((res) => {
        if (!active) return;
      ...
      .catch((err) => {
        if (!active) return;
        setError(err.response?.data?.message || 'Failed to load cleaning');
      })
  ...
  return (
    <div className="my-8">
      <h2 className="Seaction-heading px-4 text-black">Cleaning Services</h2>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="min-w-0 flex-1">
                <Link href={`/services/${s._id}`}>
                  <h3 className="text-body mb-1 font-semibold text-black transition-colors hover:text-gray-700">
                    {s.serviceName}
                  </h3>
                </Link>
                <p className="text-caption mb-2 line-clamp-2 text-gray-600">
                  {s.description || 'Professional cleaning service'}
                </p>
